...
$('#descargar').prop('hidden', true);
    ...
    $('#generarExcel').click(function(e) {
        $('#enviarAPdf').empty();
        $('.pdf').empty();
        //A partir del cliente u el id    
        
        var datosAgregar = new FormData();
        datosAgregar.append("tipoOperacion", "listarLiquidacion");
        datos.Agregar.append("id_cliente",id_cliente)
        ...
        $.ajax({
            ...
            success: function(respuesta) {
                ...
                for (var i = 0; i < contadora; i++) {
                    IDliquidaciones.push(valor0[i][0]);
                }
            }
        });
